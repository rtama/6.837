...
void drawScene(GLint program, Matrix4f V, Matrix4f P) {
    ...
    for (draw_batch b : scene.batches) {
        int start = b.start_index;
        while (start < (b.start_index + b.nindices)) {
            uint32_t index = scene.indices[start];
            recorder.record(scene.positions[index], scene.normals[index], Vector3f(scene.texcoords[index],0));
            start = start + 1;
        }
...
void freeTextures() {
   for (auto it = scene.textures.begin(); it!= scene.textures.end(); ++it) {
       std::string name = it->first;
       glDeleteTextures(1, &gltextures[name]);
   }
}
